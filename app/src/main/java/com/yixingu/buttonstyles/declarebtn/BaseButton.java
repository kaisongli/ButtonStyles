...
import com.yixingu.buttonstyles.tools.Utils;
...
public class BaseButton extends Button {
    ...
    private void init(Context context, AttributeSet attrs, int logo) {
        ...
        if (attrs.getAttributeValue("http://schemas.android.com/apk/res/android", "text") != null) {
            mIconPadding = (int) Utils.convertDpToPixel(context, 20);
        }
        ...
        for (int i = 0; i < array.getIndexCount(); ++i) {
            ...
            if (attr == R.styleable.IconButton_iconPadding) {
                mIconPadding = array.getDimensionPixelSize(attr, (int) Utils.convertDpToPixel(context, 20));
            }
            ...
            if (attr == R.styleable.IconButton_iconSize) {
                mIconSize = array.getDimensionPixelSize(attr, (int) Utils.convertDpToPixel(context, 20));
            }
            ...
            if (attr == R.styleable.IconButton_roundedCornerRadius) {
                mRoundedCornerRadius = array.getDimensionPixelSize(attr, (int) Utils.convertDpToPixel(context, 40));
            }
    ...
    private void setStyle(int color, Context context) {
        ...
        setPadding((int) Utils.convertDpToPixel(context, 30), 0, (int) Utils.convertDpToPixel(context, 30), 0);
    ...
    private void setDefaultValues(Context context, int logo) {
        ...
        mIconSize = (int) Utils.convertDpToPixel(context, 20);
        mIconCenterAligned = true;
        mRoundedCorner = false;
        mTransparentBackground = false;
        mRoundedCornerRadius = (int) Utils.convertDpToPixel(context, 40);
